...
function MangaInfo() {
    ...
    const {language: language} = useParams()
    ...
    const GetMangaVolumes = async (mangaId,offset,) => {
        ...
        const temp = await fetch(`https://api.mangadex.org/chapter?limit=100&offset=${offset}&manga=${mangaId}&translatedLanguage%5B%5D=${language}&order%5Bvolume%5D=asc`)
            .then(res => res.json());
        ...
        render(){
            ...
            return(
                <div >
                            ...
                            return (
                              <label>
                                <input
                                  ...
                                  checked = {genre === getRightLanguage(language) ? true : false}
                                  onChange={(e) => {
                    ...
                    <Button className="scrollUpButton" onClick={scrollToTop} variant='contained' color="primary" style={{fontSize:"14px"}}>Scroll To Top</Button>
